...
def get_kits_for_day(day, schedule):
    ...
    for start_day, num_kits in schedule:
        if day >= start_day: kits = num_kits
    return kits

def node_level_testing(G, block_id, num_kits):
    if num_kits == 0: return Counter(), 0
    block_nodes = [n for n, d in G.nodes(data=True) if d['block_id'] == block_id]
    tier1 = {n for n in block_nodes if G.nodes[n]['state'] == 'I'}
    tier2 = {neighbor for infected in tier1 for neighbor in G.neighbors(infected) if G.nodes[neighbor]['block_id'] == block_id and G.nodes[neighbor]['state'] in ['S', 'E', 'A']}
...
def run_simulation(strategy_name, G_template, pretrained_gnts=None):
    ...
    metrics = {"total_new_infections": 0, "total_tests_administered": 0, "total_positive_tests": 0, "total_wasted_tests": 0, "peak_infections": 0, "time_to_peak": -1, "daily_allocations": [], "first_infection_day": {b: -1 for b in range(config.NUM_BLOCKS)}, "first_intervention_day": {b: -1 for b in range(config.NUM_BLOCKS)}, "integrated_infections": 0}
    ...
    if strategy_name.startswith('LocalGNTS'):
        if pretrained_gnts:
            agent = copy.deepcopy(pretrained_gnts); agent.G = G
        else:
    ...
    for day in range(config.SIMULATION_DAYS):
        pre_step_s_count = len([n for n, d in G.nodes(data=True) if d['state'] == 'S'])
        if day >= config.TESTING_START_DAY:
            ...
            if strategy_name.startswith('LocalGNTS'):
                ...
                if strategy_name.startswith('Beta'):
                    mab = BetaBinomialMAB(config.NUM_BLOCKS); mab.update_priors(daily_test_results_buffer)
                else:
                    mab = GammaPoissonMAB(config.NUM_BLOCKS); mab.update_priors(daily_test_results_buffer, lookback_days)
                temp_counts = [Counter() for _ in range(config.NUM_BLOCKS)]
                for node, data in G.nodes(data=True): temp_counts[data['block_id']][data['state']] += 1
                ...
                for node, data in G.nodes(data=True): current_counts[data['block_id']][data['state']] += 1
                kit_allocations = {'Uniform': uniform_allocation, 'Random': random_allocation, 'Proportional': proportional_allocation}[strategy_name](config.NUM_BLOCKS, kits_today, current_counts=current_counts)
            ...
            for block_id in range(config.NUM_BLOCKS):
                ...
                num_pos = sum(pos_counts.values()); num_neg = wasted
                daily_test_results[block_id]['positive'] = num_pos; daily_test_results[block_id]['negative'] = num_neg
                metrics["total_tests_administered"] += kit_allocations[block_id]
                metrics["total_positive_tests"] += num_pos; metrics["total_wasted_tests"] += wasted
        ...
        for node, data in G.nodes(data=True): 
            ...
            if data['state'] in ['I', 'A']: total_infectious_today += 1
